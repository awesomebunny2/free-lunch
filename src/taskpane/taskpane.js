    ...
    await Excel.run(async (context) => {      
      ...
      //#region SPECIFIC TABLE VARIABLES --------------------------------------------------------------------------
        ...
        //#region VALIDATION VARIABLES ------------------------------------------------------
          ...
          //#region PROJECT TYPE TABLE VARIABLES --------------------------------------
            var projectTypeTable = context.workbook.tables.getItem("ProjectTypeTable");
            var projectTypeTableColumn = projectTypeTable.columns.getItem("Project Type");
            projectTypeTableColumn.load("items");
            var projectTypeTableRows = projectTypeTable.rows;
            projectTypeTableRows.load("items");
          //#endregion ---------------------------------------------------------------------------

          //#region PRODUCT TABLE VARIABLES ------------------------------------------------------
            var productTable = context.workbook.tables.getItem("ProductTable");
            var productTableColumn = productTable.columns.getItem("Product");
            productTableColumn.load("items");
            var productTableRows = productTable.rows
            productTableRows.load("items");
          //#endregion --------------------------------------------------------------------------

          //#region ARTIST LEAD TABLE VARIABLES -------------------------------------------------
            var artistLeadTable = context.workbook.tables.getItem("ArtistLeadTable");
            var artistLeadTableColumn = artistLeadTable.columns.getItem("Artist Lead");
            artistLeadTableColumn.load("items");
            var artistLeadTableRows = artistLeadTable.rows
            artistLeadTableRows.load("items");
          //#endregion --------------------------------------------------------------------------

          //#region QUEUE TABLE VARIABLES -------------------------------------------------------
            var queueTable = context.workbook.tables.getItem("QueueTable");
            var queueTableColumn = queueTable.columns.getItem("Queue");
            queueTableColumn.load("items");
            var queueTableRows = queueTable.rows
            queueTableRows.load("items");
          //#endregion --------------------------------------------------------------------------

          //#region TIER TABLE VARIABLES -------------------------------------------------
            var tierTable = context.workbook.tables.getItem("TierTable");
            var tierTableColumn = tierTable.columns.getItem("Tier");
            tierTableColumn.load("items");
            var tierTableRows = tierTable.rows
            tierTableRows.load("items");
          //#endregion --------------------------------------------------------------------------

          //#region TAGS TABLE VARIABLES --------------------------------------------------------
            var tagsTable = context.workbook.tables.getItem("TagsTable");
            var tagsTableColumn = tagsTable.columns.getItem("Tags");
            tagsTableColumn.load("items");
            var tagsTableRows = tagsTable.rows
            tagsTableRows.load("items");
          //#endregion --------------------------------------------------------------------------
  ...
  //#region PRODUCT AND PROJECT TYPE VARIABLE ASSIGN -------------------------------------------------------
    ...
    function productProjectTypeAssign(tableColumns, rowValues) {
      ...
      if (productInput == "Menu") {
        ...
        product = "legalLetter";
      } else if (productInput == "Envelope Insert") {
        product = "envelopeInsert";
