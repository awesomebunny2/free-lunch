...
function determineManifestPath(platform, dev) {
    ...
    if (!manifestPath) {
        throw new office_addin_usage_data_1.ExpectedError(`The manifest path was not provided.`);
    }
    if (!fs.existsSync(manifestPath)) {
        throw new office_addin_usage_data_1.ExpectedError(`The manifest path does not exist: ${manifestPath}.`);
    }
    return manifestPath;
}
...
function start(manifestPath, platform, command) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const appPlatformToDebug = start_1.parsePlatform(platform || process.env.npm_package_config_app_platform_to_debug || start_1.Platform.Win32);
            ...
            const devServer = command.devServer || (yield office_addin_cli_1.getPackageJsonScript("dev-server"));
            ...
            const packager = command.packager || (yield office_addin_cli_1.getPackageJsonScript("packager"));
            ...
            if (appPlatformToDebug === undefined) {
                throw new office_addin_usage_data_1.ExpectedError("Please specify the platform to debug.");
            }
            ...
            if (appPlatformToDebug === start_1.Platform.Android || appPlatformToDebug === start_1.Platform.iOS) {
                throw new office_addin_usage_data_1.ExpectedError(`Platform type ${appPlatformToDebug} not currently supported for debugging`);
            }
            if (!manifestPath) {
                manifestPath = determineManifestPath(appPlatformToDebug, dev);
            }
            yield start_1.startDebugging(manifestPath, {
...
function stop(manifestPath, platform, command) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const appPlatformToDebug = start_1.parsePlatform(platform || process.env.npm_package_config_app_plaform_to_debug || start_1.Platform.Win32);
            const dev = command.prod ? false : true;
